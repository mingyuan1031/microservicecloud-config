 ...
 * @since 1.1.2
 */
public abstract class LwxfAssert {
	private static Logger logger = LoggerFactory.getLogger(LwxfAssert.class);
	...
	public static void state(boolean expression, String message) {
		if (!expression) {
			if (configuration.isOnProd()) {
				logger.error(dumpAllStackTraces(message));
			} else {
	...
	private static void instanceCheckFailed(Class<?> type, Object obj, String msg) {
		...
		if (configuration.isOnProd()) {
			logger.error(dumpAllStackTraces(result));
		} else
	...
	public static String dumpAllStackTraces(String msg) {
