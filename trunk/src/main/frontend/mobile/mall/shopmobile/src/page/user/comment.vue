			...
			/*	var imgfiles = new FormData(e.target.files[0]	)*/
				...
				/*	console.log(imgfiles)*/
					_this.axios({
						...
						url: '/api/goodscomments/AAA/files',
		...
		sub: function() {
			if(this.msg.center.trim().length < 5) {
				...
				for (var i=0;i<this.msg.imgUrls.length;i++) {
					...
					imgur.push("imgList="+this.msg.imgUrls[i].id)
				...
				_this.axios({
					method: 'post',
					/*url: '/api/goodscomments/?'+imagrs+'&orderGoodsId='+_this.$route.params.ordergoodsid+'',*/
					url: '/api/goodscomments/?'+imagrs+'&orderGoodsId=41ibkt8hpw5c',
					headers: {
						'X-Requested-With': 'XMLHttpRequest',
