				...
				<form>
					<ul>
						...
						<li class="goodMlisite">
							<small>详细地址：</small>
							<span>
									<input type="text" name="" id=""  v-model="msg.detailed" value="" placeholder="如街道，楼道，门牌号等" class="leave">
								</span>
						</li>
						<li class="goodMlisite">
							<small>邮政编码：</small>
		...
		methods: {
			...
			sitebut: function() {
				...
				if(this.msg.name == "" || this.msg.name == "" || this.msg.name.trim().length <= 0|| this.msg.name.trim().length >10) {
					...
					/* if(youbian.test(this.msg.postcode.trim())==false) {
