					...
					<div v-for="(item,index) in datas" class="userTalk">
						...
						<div class="userTalk-content">
							...
							<div class="take-imgs" v-show="datas[index].files.length>0">
								...
								<ul class="two">
									<li @click="Imgsrc(index,indexdb)" v-for="(itemdb,indexdb) in datas[index].files" class="imgcenter">
							...
							<div class="praiseitem" v-show="datas[index].praises.length<2&&datas[index].praises.length>0">
								<!--"-->
							<ul v-show="datas[index].praises.length<10">
								<span><i style="height: 1rem; display: block; line-height: 1rem; font-size: 0.8rem;margin-right: 0.2rem;" class="layui-icon">&#xe6c6;</i> </span>
									<li v-for="(iteme,indexdb) in datas[index].praises">
										<div :style="{backgroundImage: 'url(' + iteme.avatar + ')'}" class="parnav">
								...
								<ul v-show="datas[index].praises.length>=12" style="width: 100%; height: 1rem; overflow: hidden;">
									<span><i style="height: 1rem; display: block; line-height: 1rem; font-size: 0.8rem;margin-right: 0.2rem;" class="layui-icon">&#xe6c6;</i> </span>
									<li v-for="(iteme,indexdb) in datas[index].praises">
										<div :style="{backgroundImage: 'url(' + iteme.avatar + ')'}" class="parnav">
				...
				_this.axios({
					...
					var obj = {
						...
						"id": res.data.data.praiseUser.id,
                         "avatar":res.data.data.praiseUser.avatar
					}
