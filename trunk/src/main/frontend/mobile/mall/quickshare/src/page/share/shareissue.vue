...
tip:{
			...
			/*	var imgfiles = new FormData(e.target.files[0]	)*/
				...
				if(this.imgData.accept.indexOf(imgstyle) == -1) {
					...
					document.getElementById("tip").innerHTML = "图片大小不能超过5M"
					 _this.setalert()
					return false;
				} else if(this.msg.imgUrls.length > 5) {
					...
					document.getElementById("tip").innerHTML = ""
					 ...
					 var formData = new FormData();
					  formData.append('files', e.target.files[0]); // 文件数据
				...
				/*	console.log(imgfiles)*/
					_this.axios({
						...
						_this.msg.imgUrls.push(res.data.data)
		...
		sub: function() {
			if(this.msg.center.trim().length < 5) {
				document.getElementById("tip").innerHTML = "帖子字数不能少于5个字";
				return false;
			} else {
