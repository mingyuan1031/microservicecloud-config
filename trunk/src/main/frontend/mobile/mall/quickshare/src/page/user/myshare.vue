<template>
	<div id="share" style="background: #EEEEEE;" class="paddingBox">
					...
					<div v-for="(item,index) in datas" class="userTalk">
						...
						<div class="userMeg">
							...
							<div class="userMeg-evaluate">
								<span @click="delindex(index)"><small>删除</small></span>
						...
						<div class="userTalk-content">
								...
								<ul class="two" >
									...
									<li v-for="(itemdb,indexdb) in datas[index].files" >
                                     <!--  <div :style="{backgroundImage: 'url(' + itemdb.path + ')'}" class="imgcenter">-->
                                     	<div class="imgcenter" :style="{backgroundImage: 'url(' + itemdb.path + ')' }" @click="Imgsrc(index,indexdb)">
							...
							<ul >
									<span> <i><img style="width: 0.9rem; height: 0.9rem; margin-right: 0.2rem;" src="../../assets/img/likeactive.png"/></i> </span>
		...
		<div class="take" id="opquick" :style="{'left':Allleft+'rem'}" style="display: none;">
		...
		<div class="delclass" v-show="delclass==true">
        	<div class="nav">
        		...
        		<div class="but">
        		    <span @click="delshare">确认</span>
        			<span @click="delshareno">取消</span>
		...
		methods: {
			...
			timeout: function() {
             ...
             delindex:function(index){
            	this.delshareindex=index
            	this.delshareid=this.datas[index].id
            	this.delclass=!this.delclass
            },
            //删除快享帖子
            delshare:function(){
            	this.alertn(5)
            	var bid=this.delshareid
            	var _this=this
            	_this.axios({
				method: 'delete',
				url: '/api/microblogs/'+bid+'',

			}).then(function(res) {
				console.log(res.data)
			   _this.datas.splice(_this.delshareindex,1)
			   _this.delclass=!_this.delclass
               _this.setalert()
			}).catch(function(err) {
				console.log(err)
				this.alertn(2, "删除失败")
		   ...
		   document.getElementsByTagName('body')[0].classList.add("oyyes")
			...
			dataNow: function() {
		    	...
		    	'ContentType':'application/json;charset=UTF-8'
					...
					url: '/api/users/0/microblogs?pageNum=' + this.pagesize + '&pageSize=20'
