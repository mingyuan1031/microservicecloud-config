...
<div style="margin: 0 auto;overflow-y: scroll;height: 750px;">
	...
	<Input v-model="value" placeholder="请输入 帖子的标题或者发帖人进行搜索..." style="width: 300px"></Input>
    <Button type="ghost" icon="ios-search">帖子搜索</Button>
	</div>
	<button @click="requestm()">dianji</button>
<!-- 表头-->
<table>
	<tr>
		...
		<td style="width: 10%">帖子状态</td>		
	</tr>
</table>
       <div class="tablex">       	      	   
       	   <div class="tablecontain">
       	   	<ul class="table_contain" v-for="(item,indexs) in trlist">
       	   			...
       	   			<div class="tupian">
       	   				...
       	   				<ul class="photos">
       	   					<li @click="tabphoto(index,indexs)" v-for="(itemx,index) in item.files">
       	   						 <img :src="itemx.path" width="50"/>
       	   					</li>     	   			
       	   				</ul>
       	   			</div>
       	   			<div class="miaoshu">
       	   				<div class="productdel">{{item.content}}<span class="knowmore" @click="colgdelite(indexs,index)">了解详情>></span></div><br />
       	   				<div class="df">
       	   					<div class="df_l">
       	   					<img src="../../../dist/img/login.ed70d3f.jpg" width="60" class="df_limg"/>
       	   					<div class="zhanghao">
       	   		...
       	   		<li style="width: 30%;padding: 10px 10px;">
       	   			<span class="conment">帖子评论内容:</span><br />
       	   			 <span class="content" v-for="chat in item.comments">{{chat.creatorname}}评论{{chat.parentname}}:{{chat.content}}<br /></span>
       	   		    <span class="knowmore">了解详情>></span>
       	   		</li>
       	   		<li style="width: 10%;text-align: left;" >
       	   			...
       	   			<span class="knowmore">更多>></span>      	   			
       	   		</li>
       	   		<li style="width: 10%;text-align: center;padding-top: 50px;">
       	   			<div class="money">
       	   			开启/关 
       	   		   </div></li>
       	   	</ul>
 ...
 export default {
            ...
            return {
            	...
            	trlist:[
            	   /*{
            	    ...
            	    tabphoto:[
            	         { tabphotourl:require("../../assets/shili.jpg")},
            	         { tabphotourl:require("../../../dist/img/login.ed70d3f.jpg")},
            	         { tabphotourl:require("../../assets/shili.jpg")},
            	         { tabphotourl:require("../../../dist/img/login.ed70d3f.jpg")},
            	         { tabphotourl:require("../../assets/shili.jpg")},
            	         { tabphotourl:require("../../../dist/img/login.ed70d3f.jpg")}
            	    ],
            	    tabphotourl:require("../../assets/tmpphoto.jpg")
            	  } */           	             	  
            ]
        ...
        methods:{
        	tabphoto(index,indexs){         		
     	     this.trlist[indexs].smallimage=this.trlist[indexs].files[index].path;
...
//      	},
        	...
        	//获取帖子数据
             ...
             var blogsArr = that.trlist;              博客数据结构
             var blogFilesTmp;                        临时博客图片容器
             var blogsFromData = res.data.data.microblogs;       博客数据骨架
             var blogTmp;                             临时博客容器
             var blogPraiseArr;                       博客点赞数据结构
             var creatorTmp;                          创建者容器
             ...
             for (var i=0;i<that.trlist.length;i++) {
             	if (that.trlist[i].praises) {
             		that.trlist[i].praisesnum=that.trlist[i].praises.length
             	} else{
             	    that.trlist[i].praisesnum=0
             	}
             	 if (that.trlist[i].comments) {
             	 	that.trlist[i].commentsnum=that.trlist[i].comments.length
             	 } else{
             	 	that.trlist[i].commentsnum=0
             	 } 
             	 ids.push(that.trlist[i].creator);
             	 if (that.trlist[i].comments) {
             	 for (var cs=0;cs<that.trlist[i].comments.length;cs++) {
             	 commentname.push(that.trlist[i].comments[cs].creator);
             	 if (that.trlist[i].comments[cs].parentCreator) {
             	 commentname.push(that.trlist[i].comments[cs].parentCreator);	
             	 } 
             	 }             	
             	 } 
			 ...
			 requestm(toLoadUserIds){
			 	var that=this;
			 	//var arrs=ids.concat(commentname)
