...
<div class="contain" style="z-index: -1;">
		...
		<div class="functions">
			<div class="search">
               <Input v-model="searchConName1" placeholder="请输入品牌名称"  style="width: 300px"></Input>
              <Button icon="ios-search" type="ghost" @click="handleSearch1">品牌搜索</Button>
               <Button  type="info" @click="modaladd=true">新增品牌</Button>
               <!--<Poptip confirm title="您确认删除这些内容吗？" @on-ok="ok" @on-cancel="cancel" placement="right">
               <Button  size="small" type="warning">删除</Button>           
               </Poptip>
		...
		<!-- 修改模态框开始-->
	    <Modal v-model="modalx" title="修改品牌信息" @on-ok="modelok" @on-cancel="modelcancel">
      ...
      <!--新增品牌-->
        <Modal v-model="modaladd" title="新增品牌信息" @on-ok="addok" @on-cancel="addcancel">
        <Form :model="formItem" :label-width="80">
       	<FormItem label="品牌名称">
            <Input v-model="addname" placeholder="请输入品牌名称..."></Input>
        </FormItem>
        <FormItem label="品牌描述">
            <Input v-model="addcontent" placeholder="请输入品牌描述..."></Input>
        </FormItem>
        </Form>
        </Modal>
        <!--新增品牌结束-->
         <!--上传logo-->
        <Modal v-model="logoadd" title="上传logo" @on-ok="logook" @on-cancel="logocancel">
        <Upload
...
var logoindex;
 ...
 name: 'searchable-table',
		...
		remove(index) {
			...
			console.log(indexs)
			a.push(deleteid)
			this.axios({ method: 'delete',url: '/api/brands/' + a})
		    ...
		    repair(index){
            	...
            	this.pdes=this.data6[index].content;
            	this.logourl= '/api/brands/'+repairid+'/files'
            },
            ...
            //添加品牌信息
            addbrands(){
            	if (this.addname==''||this.addname==null||this.addcontent==''||this.addcontent==null) {
            		this.brandsopen();
            	} else{
            		var that=this;
			       this.axios({
            ...
            newlogo(file){
            	console.log(file.data.file)
          	    this.data6[logoindex].image=file.data.file; 
          	    this.data6.splice(1,0)
            },
            //表格搜索
